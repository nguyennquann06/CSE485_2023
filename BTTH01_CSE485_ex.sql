...
create table tacgia(
ma_tgia int check (ma_tgia >= 0) not null primary key,
ten_tgia varchar(100) not null, 
hinh_tgia varchar(100));

create table theloai(
ma_tloai int check (ma_tloai >= 0) not null primary key,
ten_tloai varchar(50) not null);

create table baiviet(
ma_bviet int check (ma_bviet >= 0) not null primary key,
tieude varchar(200) not null,
ten_bhat varchar(100) not null,
ma_tloai int check (ma_tloai >= 0) not null, foreign key (ma_tloai) references theloai(ma_tloai),
tomtat text not null,
noidung text,
ma_tgia int check (ma_tgia >= 0) not null, foreign key (ma_tgia) references tacgia(ma_tgia),
ngayviet datetime not null default 'Ngày hiện tại từ HT',
hinhanh varchar(200));
